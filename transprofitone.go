...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
    ...
    //设计合约
    if args[0]=="transprofitone"{
        return t.transprofitone(stub,args)
    }
...
func (t *SimpleChaincode) transprofitone(stub shim.ChaincodeStubInterface,args []string)pb.Response{
    if len(args)!=8{
    ...
    //     return shim.Error("contractN must be c3002")
    // }
    //获得合约持仓
    ...
    if contractHold.ContractStatus=="9"&&actionType=="10"{
        netAccountStr,result:=getNetAmountByAcctIdAndContractN(stub,"c3001",accIdStr,chaincodeToCall,channelId)
        if result!=success{
            return shim.Error(result)
        }
        kk,err:=strconv.Atoi(netAccountStr)
        if err!=nil{
        ...
        if zzBytes!=nil{
            zz,err=strconv.Atoi(string(zzBytes))
            if err!=nil{
        ...
        if gg>0{
            //B向A转钱
            result=moveMoney(stub,contractHold.AccIdB,contractHold.AccIdA,strconv.Itoa(gg),contractNStr,chaincodeToCall,channelId)
            if result!=success{
                return shim.Error(result)
            }
        }else if gg<0{
            //A向B转钱
            ggStr:=strconv.Itoa(gg)
            ggFloat64,err:=strconv.ParseFloat(ggStr,64)
            if err!=nil{
                return shim.Error("ggStr ParseFloat failed")
            }
            gg=int(math.Abs(ggFloat64))
            result=moveMoney(stub,contractHold.AccIdA,contractHold.AccIdB,strconv.Itoa(gg),contractNStr,chaincodeToCall,channelId)
            if result!=success{
                return shim.Error(result)
            }
        }
