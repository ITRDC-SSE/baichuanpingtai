...
func (t *SimpleChaincode) transprofittwo(stub shim.ChaincodeStubInterface,args []string)pb.Response{
    ...
    if contractNStr!=contractN{
        return shim.Error("contractN must be c3004")
    }
    //获得合约持仓
    ...
    if contractHold.ContractStatus=="9"&&actionType=="10"{
        netAccountStr,result:=getNetAmountByAcctIdAndContractN(stub,"c3003",contractHold.AccIdB,chaincodeToCall,channelId)
        if result!=success{
            return shim.Error(result)
        }
        ...
        price,result:=getStockPrice(stub,time,"sh0003",chaincodeToCall,channelId)
        if result!=success{
            return shim.Error(result)
        }
        transMoney:=(price/50)*gg
