...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
    ...
    //设计合约
    if args[0]=="lendingpro"{
        return t.lendingpro(stub,args)
    }
...
func (t *SimpleChaincode) lendingpro(stub shim.ChaincodeStubInterface,args []string)pb.Response{
    if len(args)!=10{
    ...
    if corContractNAHoldStr!=""{
        ...
        //(accFlowId,acctAssetA,acctAssetB,sseHoldA,sseHoldB)构成allData
        //获得最新的账户流水编号
        ...
        if actionType=="7"&&corContractNAHold.ContractStatus=="6"{
            if contractHold.ContractStatus=="0"&&ti>=20170110&&ti<=20170115{
                if acctAssetA.AvaMoney<72000{
                ...
                //A给B转钱
                acctAssetA.AvaMoney-=72000
                acctAssetB.AvaMoney+=72000
                //账户流水
                ...
                if contractHold.CorContractNB!=""{
                    ...
                    if response.Status!=shim.OK{
                        errStr := fmt.Sprintf("222 getContractHold Failed to invoke chaincode. Got error: %s", string(response.Payload))
                        fmt.Printf(errStr)
        ...
        if contractHold.ContractStatus=="0"&&actionType=="1"&&ti>=20170110&&ti<=20170115{    
            ...
            if contractHold.CorContractNB!=""{
                ...
                response := stub.InvokeChaincode(corContractHoldB.ContractCCID, invokeArgs, channelId)
                if response.Status!=shim.OK{
                    errStr := fmt.Sprintf("111 getContractHold Failed to invoke chaincode. Got error: %s", string(response.Payload))
                    fmt.Printf(errStr)
                    return shim.Error(errStr+corContractHoldB.ContractCCID+corContractHoldB.ContractFunctionName+";"+string(contractHoldBytes)+";"+allData)
                }
            }
            contractHold.ContractStatus="2"
        }else if contractHold.ContractStatus=="2"&&actionType=="8"&&ti>=20170710&&ti<=20170811{
            if acctAssetB.AvaMoney<68000{
                return shim.Error("acctB do not have enough money")
            }
            //B给A转钱
            acctAssetA.AvaMoney+=68000
            acctAssetB.AvaMoney-=68000
            //A给B转券
            sseHoldA.HoldNum-=1000
            sseHoldB.HoldNum+=1000
            //钱账户流水
