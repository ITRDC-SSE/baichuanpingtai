...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	//设计合约
	if args[0]=="bigoptionone"{
		return t.bigoptionone(stub,args)
	}
...
func (t *SimpleChaincode) bigoptionone(stub shim.ChaincodeStubInterface,args []string)pb.Response{
	if len(args)!=8{
	...
	if contractHold.ContractStatus=="0"&&actionType=="1"{
        ...
        if err!=nil{
        	return shim.Error("currTimeStr atoi failed")
        }
		...
		//判断A的钱是否足够
		if acctAssetA.AvaMoney<60000{
			return shim.Error("there is not enough money")
		}
        //锁B的券
        invokeArgs=util.ToChaincodeArgs("invoke","lockStock",contractHold.AccIdB,contractNStr,"sh0003","1000")
		response = stub.InvokeChaincode(chaincodeToCall, invokeArgs, channelId)
		if response.Status!=shim.OK{
			if response.Status!=shim.OK{
		    	errStr := fmt.Sprintf("lockStock Failed to invoke chaincode. Got error: %s", string(response.Payload))
				fmt.Printf(errStr)
				return shim.Error(errStr)
		    }
	    }
	    //锁券成功就转钱,A给B转钱
        acctAssetA.AvaMoney-=60000
        acctAssetB.AvaMoney+=60000
        //账户流水信息
